...
class WOFComputerPlayer(WOFPlayer):
    ...
    def getPossibleLetters(self,guessed):
        ...
        for s in LETTERS:
            if s not in guessed:
                guess_list.append(s)
        if VOWEL_COST < 250:
            return list(set(guess_list) - set(VOWELS))
        else: return guess_list
    ...
    def getMove(self,category, obscuredPhrase, guessed):
        ...
        else: return random.choice(self.SORTED_FREQUENCIES)
...
# %%
""" TEST code for py3 specilization
    C5
"""
...
file_name = r"./readonly/small_img.zip"
with zipfile.ZipFile(file_name) as zip:
    zip.extractall()
#     info = zip.infolist()
    f_names = zip.namelist()
#     for fname in f_names:
image=Image.open(f_names[0])
#         display(im)
...
for txt in text_lines:
    if search_str.upper() in txt:
        print(f"{search_str} is in image at line : \n\n{txt}")